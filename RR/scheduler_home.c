...
#include <sys/wait.h>
#include <string.h>
#define INTERVAL 1 //sec, milisec doesnt work with alarm
...
void get_pid(int sig, siginfo_t *info, void *context)
{
    ...
    else if(sig == SIGPWR)
    {
    	...
    	if(busy==0)
    	{
		    ...
		    for(i=0; i<N; i++)
		    {
		    	if(pr[i].state > 0 && pr[i].state < min)
				{
					min = pr[i].state;
					next = i;
				}
